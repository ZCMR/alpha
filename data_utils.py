    ...
    with open(data_path) as infile:
        ...
        for d in data:
            for aspect in d['aspects']:
                ...
                post1 = [i-aspect['from'] for i in range(aspect['from'])] \
                       +[0 for _ in range(aspect['from'], aspect['to'])] \
                       +[i-aspect['to']+1 for i in range(aspect['to'], length)]
                post = [1 - sigmoid_alpha(aspect['from'] - i - 1, 10, aspect['from']) for i in range(aspect['from'])] + [0 for _ in range(aspect['from'], aspect['to'])] + [
                    1 - sigmoid_alpha(i - aspect['to'], 10, length-aspect['to']+1) for i in range(aspect['to'], length)]
                # aspect mask
                if len(asp) == 0:
                ...
                sample = {'text': tok, 'aspect': asp, 'pos': pos, 'post': post, 'head': head,\
                          ...
                          'aspect_post': aspect_post, 'text_list': text_list,'post1':post1}
...
class Tokenizer(object):
    ...
    def text_to_sequence_asp(self, text, reverse=False, padding='post', truncating='post'):
        if self.lower:
            text = text.lower()
        words = Tokenizer.split_text(text)
        sequence = [self.vocab.word_to_id(w) for w in words]
        ...
        if reverse:
            sequence.reverse()
        return Tokenizer.pad_sequence(sequence, pad_id=self.vocab.pad_id, maxlen=10,
                                      padding=padding, truncating=truncating)
...
class SentenceDataset(Dataset):
    ...
    def __init__(self, fname, tokenizer, opt, vocab_help):
        ...
        for obj in tqdm(parse(fname), total=len(parse(fname)), desc="Training examples"):
            ...
            aspect = tokenizer.text_to_sequence_asp(obj['aspect'])  # max_length=10
            # print('237',obj['post'])
            post2 = [post_vocab.stoi.get(t, post_vocab.unk_index) for t in obj['post']]
            # print('239',post)
            post = obj['post']
            len_post = len(post)
            x = (np.zeros(opt.max_length) + opt.pad_id).astype('float32')
            x[:len_post] = post

            post = tokenizer.pad_sequence(post2, pad_id=opt.pad_id, maxlen=opt.max_length, dtype='int64', padding='post', truncating='post')
            #print('240',x,post)
            ...
            #print('271',obj['post1'])
            post1 = [post_vocab.stoi.get(t, post_vocab.unk_index) for t in obj['post1']]
            # print('274',post1)
            post1 = tokenizer.pad_sequence(post1, pad_id=opt.pad_id, maxlen=opt.max_length, dtype='int64', padding='post', truncating='post')
            # print('275',post1)
            data.append({
                'text': text, 
                'aspect': aspect, 
